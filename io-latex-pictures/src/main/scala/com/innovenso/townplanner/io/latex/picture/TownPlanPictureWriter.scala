...
case class TownPlanPictureWriter(assetRepository: AssetRepository) {
  ...
  def views(townPlan: TownPlan): List[_ <: CompiledView[_ <: View]] =
    townPlan.businessCapabilityMaps ++ townPlan.fullTownPlanViews ++ townPlan.projectMilestoneImpactViews ++ townPlan.technologyRadars ++ townPlan.architectureDecisionRecords ++ townPlan.knowledgeMatrices ++ townPlan.projectMilestoneOverviews
}
