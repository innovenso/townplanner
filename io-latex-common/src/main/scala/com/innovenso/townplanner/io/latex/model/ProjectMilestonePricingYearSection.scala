...
case class ProjectMilestonePricingYearSection(
    milestone: ProjectMilestoneDecorator,
    fiscalYear: Year,
  ...
  val bodyRows: List[LatexBodyRow] = milestone.milestone
    .costs(fiscalYear)
    .map(cost =>
      LatexBodyRow(
        List(
  ...
  val currencies: List[Currency] =
    milestone.milestone.costs.map(_.totalCost.currency).distinct
  ...
  val footerRows: List[LatexFooterRow] = currencies.map(currency =>
    LatexFooterRow(
      List(
        ...
        LatexEmptyCell,
        LatexTextCell(
          amount(milestone.milestone.totalCapex(fiscalYear, currency))
        ),
        LatexTextCell(
          amount(milestone.milestone.totalOpex(fiscalYear, currency))
        ),
        LatexEmptyCell
      )
    )
