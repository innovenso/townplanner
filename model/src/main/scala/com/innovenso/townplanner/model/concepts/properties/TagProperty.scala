...
case class TagProperty(key: Key = Key(), sortKey: SortKey = SortKey.next, tagKey: Key) extends Property  {
...
trait HasTagProperties extends HasProperties {
  ...
  def withTag(tagProperty: TagProperty): HasProperties = withProperty(tagProperty)
...
trait CanConfigureTagProperties[ModelComponentType <: HasTagProperties] {
  ...
  def isTagged(tag: Tag): HasTagProperties = propertyAdder.withProperty(modelComponent, TagProperty(tag))
}
