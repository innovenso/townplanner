...
class SelligentBirthdayCampaignSpec extends AnyFlatSpec with GivenWhenThen {
  "the proposed solution" can "be rendered" in new DiagramIO {
    ...
    And("Selligent")
    val selligent: ItSystem = ea describes ItSystem(title = "Selligent") as {
      it =>
        it has Description(
          "Marketing automation platform, mainly used for segmented email campaigns"
        )
    ...
    And("NGW")
    val ngw: ItSystem =
      ea describes ItSystem(title = "NGW - Napoleon Games Website") as { it =>
        it has Description(
          "The Belgian Napoleon Games Website, serving 4 domains: the main napoleongames.be domain, as well as napoleoncasino.be, napoleondice.be and napoleonsports.be"
        )
        it isUsedBy player
      }
    ...
    val eventBus: ItSystem =
      ea describes ItSystem(title = "Napoleon Games Event Bus") as { it =>
        it has Description(
          "RabbitMQ used for all events across the NGW platform"
        )
      }

    And("The BGC and its integration service")
    val bgc: ItSystem =
      ea describes ItSystem(title = "Belgian Gaming Commission") as { it =>
        it has Description("The BGC exposes a SOAP API for us to use")
      }

    val bgcIntegrationSystem: ItSystem =
      ea describes ItSystem(title = "BGC Integration Service") as { it =>
        it has Description(
          "Anti-corruption layer between NGW and the Belgian Gaming Commission. Mostly an empty box right now, with all the logic still in the NGW Monolith"
        )
        it uses bgc
      }

    val bgcIntegrationMicroservice: Microservice =
      ea describes Microservice(title = "BGC Microservice") as { it =>
    ...
    val diPlatform: ItSystem =
      ea describes ItSystem(title = "Data Intelligence Platform") as { it =>
        it uses (ngw, "extracts operational data")
        it uses (selligent, "uploads player profiles")
      }

    And("The Campaign Tool")
    val campaignTool: ItSystem =
      ea describes ItSystem(title = "Campaign Tool") as { it =>
        it has Description(
          "In-house developed tool for managing marketing campaigns, in 3 languages"
        )
      }

    val communicationWorker: Microservice =
      ea describes Microservice(title = "Communication Worker") as { it =>
        it isPartOf campaignTool
        it has Description(
          "Translates campaign events into communications to customers"
        )
    ...
    val communicationService: Microservice =
      ea describes Microservice(title = "Communication Service") as { it =>
        it isPartOf campaignTool
        it has Description(
          "Anti-corruption layer for a number of communication systems"
        )
    ...
    val campaignService: Microservice =
      ea describes Microservice(title = "Campaign Service") as { it =>
        it isPartOf campaignTool
    ...
    val campaignDatabase: Database =
      ea describes Database(title = "Campaign Database") as { it =>
    ...
    val campaignApiGateway: Gateway =
      ea describes Gateway(title = "Campaign API Gateway") as { it =>
        it has Description(
          "API Gateway, exposing the Campaign API to external systems"
        )
        it isPartOf campaignTool
    ...
    val bgcFlowView: FlowView =
      ea needs FlowView(title = "BGC Check for Birthday people") and { it =>
        it has Request("daily BGC cron job") from ngw to ngw
        it has Request(
          "request BGC check for people with birthday tomorrow"
        ) from ngw to bgcIntegrationMicroservice
        it has Request(
          "requests bulk BGC check"
        ) from bgcIntegrationMicroservice to bgc
        ...
        it has Request(
          "fetches operational data, including BGC status"
        ) from diPlatform to ngw
        it has Message(
          "sends player profile data, including BGC status"
        ) from diPlatform to selligent
      }

    val birthdayFlowView: FlowView =
      ea needs FlowView(title = "Birthday email campaign including bonus") and {
        it =>
          it has Request(
            "select players with birthday today and BGC check passed less than 24 hours ago"
          ) from selligent to selligent
          it has Request("send birthday email") from selligent to player
          it has Request(
            "request reward for player"
          ) from selligent to campaignApiGateway
          it has Request(
            "pass on reward request"
          ) from campaignApiGateway to campaignService
