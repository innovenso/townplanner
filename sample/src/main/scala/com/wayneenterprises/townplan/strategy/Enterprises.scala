...
case class Enterprises()(implicit ea: EnterpriseArchitecture) {
  val wayneCorp: Enterprise = ea describes Enterprise(title = "Wayne Enterprises") as { it =>
  ...
  ea needs BusinessCapabilityMap(forEnterprise = wayneCorp.key)
  ea needs FullTownPlanView(forEnterprise = wayneCorp.key)
}
