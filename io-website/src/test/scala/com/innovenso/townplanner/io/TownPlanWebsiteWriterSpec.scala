...
class TownPlanWebsiteWriterSpec extends AnyFlatSpec with GivenWhenThen {
  ...
  "Documentation website" should "be written" in new WebsiteIO {
    ...
    val cap1: BusinessCapability = samples.capability(
      servedEnterprise = Some(enterprise),
      tags = List(tag1, tag2)
    )
    val cap11: BusinessCapability =
      samples.capability(parentCapability = Some(cap1))
    val cap2: BusinessCapability =
      samples.capability(servedEnterprise = Some(enterprise), tags = List(tag2))
    ...
    val system1: ItSystem = samples.system(
      realizedBuildingBlock = Some(bb1),
