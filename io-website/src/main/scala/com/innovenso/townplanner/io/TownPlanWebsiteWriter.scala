...
import com.innovenso.townplanner.io.model.{HtmlSpecification, NavigationContext, NavigationPages, NavigationTree, StaticAssetSpecification, StyleOrImageAssetSpecification}
...
import website.html.{ElementPage, Index}
...
case class TownPlanWebsiteWriter()(implicit assetRepository: AssetRepository) {
  ...
  private def copyClassPathResource(path: String, targetFileName: String): Unit = {
    ...
    val specification = StyleOrImageAssetSpecification(targetFile, targetFileName)
  ...
  def pages()(implicit outputContext: OutputContext, townPlan: TownPlan): List[Output] = {
  ...
  def navigation()(implicit outputContext: OutputContext, townPlan: TownPlan): Unit = {
    NavigationPages(townPlan).pages.foreach(page => {
      val outputFile =
        File.createTempFile(UUID.randomUUID.toString, Html.extension )
      val html = Index(page, townPlan).body
      println("writing index page: " + page.path)
